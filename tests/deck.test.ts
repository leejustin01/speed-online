...
test("dealCards deals the correct amount of cards to each element", () => {
    ...
    const { players, drawPiles, playPiles } = dealCards(playersArray, deck)
    ...
    expect(drawPiles[0]).toHaveLength(5)
    expect(drawPiles[1]).toHaveLength(5)
    expect(playPiles[0]).toHaveLength(1)
    expect(playPiles[1]).toHaveLength(1)
})